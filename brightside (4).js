...
(function initBrightsideSecret(titleEl, fallTargets) {
      ...
      player = new YT.Player('bsYT', {
        ...
        events: {
          onReady: (e) => {
            ...
            // an async gap. Start muted, get it playing, then unmute — unmuting
            // an already-playing video is permitted.
            e.target.mute();
            e.target.playVideo();
            let tries = 0;
            ...
            // iOS fallback: some iOS versions never fire a clean PLAYING state
            // change through the API. Poll getPlayerState directly and start the
            // schedule as soon as we see playback, so lyrics aren't stuck waiting.
            let sTries = 0;
            const startPoll = setInterval(() => {
              sTries++;
              const st = e.target.getPlayerState && e.target.getPlayerState();
              if (st === YT.PlayerState.PLAYING && !started) {
                started = true;
                startSchedule();
          ...
          onStateChange: (e) => {
            // Start the lyric schedule off a plain wall-clock timer the moment
            // playback begins. We do NOT poll getCurrentTime() every frame (it
            // lags the real audio); each word is queued with setTimeout.
  ...
  // ---------- lyric scheduling (wall-clock from play start) ----------
  let scheduleTimers = [];
  function startSchedule() {
    // clear any prior timers (e.g. if replayed)
    scheduleTimers.forEach((id) => clearTimeout(id));
    scheduleTimers = [];
    const t0 = performance.now();
    wordEvents.forEach((ev) => {
      const delayMs = ev.t * 1000; // ev.t already in seconds from song start
      const id = setTimeout(() => showWord(ev), Math.max(0, delayMs));
  ...
  // (Lyric timing is handled by startSchedule() above — no per-frame polling.)
