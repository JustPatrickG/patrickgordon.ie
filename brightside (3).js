...
(function initBrightsideSecret(titleEl, fallTargets) {
  ...
  function buildOverlay() {
    ...
    overlay.innerHTML = `
      ...
      <div id="bsDebug" style="position:fixed;top:1.5rem;right:1.5rem;color:rgba(255,255,255,0.5);font-family:var(--font);font-size:0.7rem;z-index:60;"></div>
    `;
    ...
    overlay.querySelector('#bsNudge').addEventListener('click', () => {
      // Lyrics running late? Each tap pulls them 1s EARLIER (adds to the clock
      // the engine reads, so it thinks more of the song has elapsed).
      nudge += 1.0;
      const btn = overlay.querySelector('#bsNudge');
      btn.textContent = 'sync +' + nudge.toFixed(0) + 's';
    });
  }
  ...
  function tick() {
    ...
    const dbg = document.getElementById('bsDebug');
    if (dbg) {
      const nextT = nextIdx < wordEvents.length ? wordEvents[nextIdx].t.toFixed(1) : '-';
      dbg.textContent = 'clock ' + t.toFixed(1) + 's | next ' + nextT + 's';
    }
