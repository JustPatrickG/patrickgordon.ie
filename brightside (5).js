  ...
  function onPlay() {
    ...
    loadYT(() => {
      player = new YT.Player('bsYT', {
        ...
        events: {
          onReady: (e) => {
            // Muted autoplay is always allowed (incl. iOS). Start muted + playing,
            // then unmute once it's actually rolling — unmuting a playing video
            // is permitted, which is what gets audio through on iPhone.
            e.target.mute();
            e.target.playVideo();
            ...
            const kick = setInterval(() => {
              tries++;
              let st = -1;
              try { st = e.target.getPlayerState(); } catch (err) {}
              if (st === YT.PlayerState.PLAYING) {
                try { e.target.unMute(); e.target.setVolume(90); } catch (err) {}
                if (!started) { started = true; startSchedule(); }
                clearInterval(kick);
              } else if (tries > 40) {
                // last resort: unmute + start anyway
                try { e.target.unMute(); e.target.setVolume(90); e.target.playVideo(); } catch (err) {}
                if (!started) { started = true; startSchedule(); }
                clearInterval(kick);
              }
            }, 100);
