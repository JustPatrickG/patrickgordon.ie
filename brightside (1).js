...
(function initBrightsideSecret(titleEl, fallTargets) {
  ...
  // Global sync offset (seconds). The first vocal line lands at 21.0s in my
  // timing table but the actual video's vocals start at ~7s, so shift every
  // timestamp earlier by 14s (21 - 7). Tweak if needed: increase to make
  // lyrics appear EARLIER, decrease to make them LATER.
  const TIME_OFFSET = 14.5;
